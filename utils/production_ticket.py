...
class ProductionTicketPDF(FPDF):
    def __init__(self):
        ...
        if not font_regular or not font_bold:
            ...
            self.add_font('DejaVu', '', 'Helvetica')
            self.add_font('DejaVu', 'B', 'Helvetica-Bold')
            self.add_font('DejaVu', 'I', 'Helvetica-Oblique')  # fallback for italic
        else:
    ...
    def ticket(self, order, client, order_items, y_offset, table_full_width=False):
        ...
        # --- POPRAWIONY ADRES DOSTAWY - DANE Z ZAMÓWIENIA MAJĄ PRIORYTET ---
        self.set_font("DejaVu", "B", 7)
        self.set_xy(x + 60, y_klient)
        
        # Sprawdź najpierw dane z zamówienia, potem z klienta
        delivery_company = (getattr(order, "delivery_company", "") or 
                           getattr(client, "delivery_company", "") or 
                           getattr(client, "name", ""))
        
        delivery_street = (getattr(order, "delivery_street", "") or 
                          getattr(client, "delivery_street", "") or 
                          getattr(client, "street", ""))
        
        delivery_postal_code = (getattr(order, "delivery_postal_code", "") or 
                               getattr(client, "delivery_postal_code", "") or 
                               getattr(client, "postal_code", ""))
        
        delivery_city = (getattr(order, "delivery_city", "") or 
                        getattr(client, "delivery_city", "") or 
                        getattr(client, "city", ""))
        
        # POPRAWKA: OSOBA KONTAKTOWA I TELEFON TAKŻE Z ZAMÓWIENIA
        contact_person = (getattr(order, "delivery_contact_person", "") or 
                         getattr(client, "contact_person", ""))
        
        contact_phone = (getattr(order, "delivery_phone", "") or 
                        getattr(client, "phone", ""))
...
def generate_production_ticket_from_json(json_file_path, output_path):
    """Generuje PDF zlecenia produkcyjnego z danych JSON."""
    import json
    ...
    try:
        ...
        # Tworzenie obiektów podobnych do Django models
        class OrderObj:
            def __init__(self, data_dict):
                # Mapowanie pól z bazy danych
                self.order_number = data_dict.get('order_number', '')
                self.order_date = data_dict.get('order_date', '')
                self.delivery_date = data_dict.get('delivery_date', '')
                self.notes = data_dict.get('notes', '')
                self.delivery_company = data_dict.get('delivery_company', '')
                self.delivery_street = data_dict.get('delivery_street', '')
                self.delivery_postal_code = data_dict.get('delivery_postal_code', '')
                self.delivery_city = data_dict.get('delivery_city', '')
                # Dodaj inne pola z obiektu order jeśli potrzebne
        ...
        class ClientObj:
            def __init__(self, data_dict):
                # Mapowanie pól klienta z bazy danych
                self.name = data_dict.get('name', '')
                self.client_number = data_dict.get('client_number', '')
                self.contact_person = data_dict.get('contact_person', '')
                self.phone = data_dict.get('phone', '')
                self.email = data_dict.get('email', '')
                self.street = data_dict.get('street', '')
                self.postal_code = data_dict.get('postal_code', '')
                self.city = data_dict.get('city', '')
                self.nip = data_dict.get('nip', '')
                # Dodaj inne pola z obiektu client jeśli potrzebne
                for key, value in data_dict.items():
                    if not hasattr(self, key):
                        setattr(self, key, value)
        ...
        # Tworzenie obiektów
        order_obj = OrderObj(order_data)
        client_obj = ClientObj(client_data)
        orderitem_objs = [OrderItemObj(item) for item in order_items]
        
        # Generuj PDF używając istniejącej funkcji
        export_production_ticket(order_obj, client_obj, orderitem_objs, output_path)
        print(f"PDF zlecenia produkcyjnego wygenerowany pomyślnie: {output_path}")
        return True
        
    except Exception as e:
        print(f"Błąd podczas generowania PDF zlecenia produkcyjnego: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Użycie: python production_ticket.py <plik_json> <ścieżka_wyjściowa_pdf>")
        sys.exit(1)
    
    json_file = sys.argv[1]
    output_file = sys.argv[2]
    
    success = generate_production_ticket_from_json(json_file, output_file)
