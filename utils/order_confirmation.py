...
class PDFGenerator:
    ...
    def generate_pdf(self, filename="output.pdf"):
        ...
        # Szukaj czcionek w różnych lokalizacjach
        folder = os.path.dirname(__file__)
        app_dir = os.path.dirname(os.path.dirname(folder))  # Katalog aplikacji
        font_locations = [
            folder,  # Lokalizacja skryptu
            os.path.dirname(folder),  # Katalog nadrzędny
            app_dir,  # Katalog aplikacji
            os.path.join(app_dir, "resources"),  # Katalog resources w aplikacji
            "C:/ProgramC/resources"  # Sztywna ścieżka do zasobów projektu
        ]
        ...
        for location in font_locations:
            regular_path = os.path.join(location, "DejaVuSans.ttf")
            bold_path = os.path.join(location, "DejaVuSans-Bold.ttf")
            if os.path.exists(regular_path) and os.path.exists(bold_path):
                font_regular = regular_path
                font_bold = bold_path
                print(f"Znaleziono czcionki w: {location}")
                break
        
        if not font_regular or not font_bold:
            print("BŁĄD: Nie znaleziono plików czcionek!")
            print(f"Sprawdzone lokalizacje: {font_locations}")
            # Użyj czcionek wbudowanych jeśli nie znaleziono TTF
            pdf.add_font('DejaVu', '', 'Helvetica')
            pdf.add_font('DejaVu', 'B', 'Helvetica-Bold')
        else:
            print(f"Używam czcionek: {font_regular} i {font_bold}")
        ...
        # --- Adres dostawy, jeśli nie podano, to dane firmy + osoba kontaktowa i telefon ---
        ...
        # ZMIANA TUTAJ: Zmniejszenie odstępu przed tabelą produkcji
        pdf.ln(40) # Poprzednio było pdf.ln(74)
        ...
        uwagi_text = format_pdf_value(uwagi)
        staly_tekst = "Prosimy o zwrotne potwierdzenie zamówienia. Potwierdzenie jest zgodą na zlecenie produkcyjne."
        if uwagi_text.strip():
            pdf.multi_cell(176, 6, uwagi_text, align="L")
            pdf.ln(1)
        pdf.set_font("DejaVu", "", 10)  # stały tekst większą czcionką
        pdf.multi_cell(176, 6, staly_tekst, align="L")
        pdf.set_font("DejaVu", "", 8)  # powrót do domyślnej czcionki
        pdf.ln(6)

        # --- Stopka i tekst 8 pkt ---
        # Dodany tekst 8 pkt, wycentrowany, maksymalnie w dół
        # QR kod nad tekstem
        qr_data = f"Nr zamówienia: {self.order.get('Nr zamówienia', '')}\nNr klienta: {self.order.get('Nr klienta', '')}\nData wysyłki: {self.order.get('Data dostawy', '')}"
        ...
        img.save(buffer, "PNG")
        buffer.seek(0)
        # Pozycja QR: 18 mm od dołu, wyśrodkowany
        qr_width_mm = 24 * 1.3  # powiększenie o 30%
        qr_x = (210 - qr_width_mm) / 2  # A4 szerokość 210mm
        qr_y = pdf.h - 18 - qr_width_mm - 2  # 2mm odstępu nad tekstem
        pdf.image(buffer, x=qr_x, y=qr_y, w=qr_width_mm, h=qr_width_mm, type="PNG")
        # Dodany tekst 8 pkt, wycentrowany, maksymalnie w dół
        ...
        # Nie otwieraj automatycznie - aplikacja C++ sama otwiera plik
...
def export_order_to_pdf(order, client, order_items, filename):
    ...
    # POPRAWIONA SEKCJA ADRESU DOSTAWY - DANE Z ZAMÓWIENIA MAJĄ PRIORYTET
    adres_dostawy = {}
    
    # Najpierw sprawdź dane z zamówienia (order)
    if isinstance(order, dict):
        if order.get("delivery_company"):
            adres_dostawy["firma"] = order.get("delivery_company")
        elif client_dict.get("delivery_company"):
            adres_dostawy["firma"] = client_dict["delivery_company"]
        elif client_dict.get("Firma"):
            adres_dostawy["firma"] = client_dict["Firma"]
        if order.get("delivery_street"):
            adres_dostawy["ulica"] = order.get("delivery_street")
        elif client_dict.get("delivery_street"):
            adres_dostawy["ulica"] = client_dict["delivery_street"]
        elif client_dict.get("Ulica i nr"):
            adres_dostawy["ulica"] = client_dict["Ulica i nr"]
        if order.get("delivery_postal_code"):
            adres_dostawy["kod_pocztowy"] = order.get("delivery_postal_code")
        elif client_dict.get("delivery_postal_code"):
            adres_dostawy["kod_pocztowy"] = client_dict["delivery_postal_code"]
        elif client_dict.get("Kod pocztowy"):
            adres_dostawy["kod_pocztowy"] = client_dict["Kod pocztowy"]
        if order.get("delivery_city"):
            adres_dostawy["miejscowość"] = order.get("delivery_city")
        elif client_dict.get("delivery_city"):
            adres_dostawy["miejscowość"] = client_dict["delivery_city"]
        elif client_dict.get("Miasto"):
            adres_dostawy["miejscowość"] = client_dict["Miasto"]
        if order.get("delivery_contact_person"):
            adres_dostawy["osoba_kont"] = order.get("delivery_contact_person")
        elif client_dict.get("Osoba kontakt."):
            adres_dostawy["osoba_kont"] = client_dict["Osoba kontakt."]
        if order.get("delivery_phone"):
            adres_dostawy["telefon"] = order.get("delivery_phone")
        elif client_dict.get("Nr telefonu"):
            adres_dostawy["telefon"] = client_dict["Nr telefonu"]
    else:
        if hasattr(order, 'delivery_company') and order.delivery_company:
            adres_dostawy["firma"] = order.delivery_company
        elif client_dict.get("delivery_company"):
            adres_dostawy["firma"] = client_dict["delivery_company"]
        elif client_dict.get("Firma"):
            adres_dostawy["firma"] = client_dict["Firma"]
        if hasattr(order, 'delivery_street') and order.delivery_street:
            adres_dostawy["ulica"] = order.delivery_street
        elif client_dict.get("delivery_street"):
            adres_dostawy["ulica"] = client_dict["delivery_street"]
        elif client_dict.get("Ulica i nr"):
            adres_dostawy["ulica"] = client_dict["Ulica i nr"]
        if hasattr(order, 'delivery_postal_code') and order.delivery_postal_code:
            adres_dostawy["kod_pocztowy"] = order.delivery_postal_code
        elif client_dict.get("delivery_postal_code"):
            adres_dostawy["kod_pocztowy"] = client_dict["delivery_postal_code"]
        elif client_dict.get("Kod pocztowy"):
            adres_dostawy["kod_pocztowy"] = client_dict["Kod pocztowy"]
        if hasattr(order, 'delivery_city') and order.delivery_city:
            adres_dostawy["miejscowość"] = order.delivery_city
        elif client_dict.get("delivery_city"):
            adres_dostawy["miejscowość"] = client_dict["delivery_city"]
        elif client_dict.get("Miasto"):
            adres_dostawy["miejscowość"] = client_dict["Miasto"]
        if hasattr(order, 'delivery_contact_person') and order.delivery_contact_person:
            adres_dostawy["osoba_kont"] = order.delivery_contact_person
        elif client_dict.get("Osoba kontakt."):
            adres_dostawy["osoba_kont"] = client_dict["Osoba kontakt."]
        if hasattr(order, 'delivery_phone') and order.delivery_phone:
            adres_dostawy["telefon"] = order.delivery_phone
        elif client_dict.get("Nr telefonu"):
            adres_dostawy["telefon"] = client_dict["Nr telefonu"]
...
def generate_order_confirmation_from_json(json_file_path, output_path):
    """Generuje PDF potwierdzenia zamówienia z danych JSON."""
    import json
    ...
    try:
        ...
        # Konwertuj format danych z C++ na format Python
        # Mapowanie pól z bazy danych na format używany przez generator PDF
        order_dict = {
            'Nr zamówienia': order_data.get('order_number', ''),
            'Data zamówienia': order_data.get('order_date', ''),
            'Data dostawy': order_data.get('delivery_date', ''),
            'Uwagi': order_data.get('notes', ''),
        }
        
        client_dict = {
            'Firma': client_data.get('name', ''),
            'Nr klienta': client_data.get('client_number', ''),
            'Osoba kontakt.': client_data.get('contact_person', ''),
            'Nr telefonu': client_data.get('phone', ''),
            'E-mail': client_data.get('email', ''),
            'Ulica i nr': client_data.get('street', ''),
            'Kod pocztowy': client_data.get('postal_code', ''),
            'Miasto': client_data.get('city', ''),
            'NIP': client_data.get('nip', ''),
            'delivery_company': order_data.get('delivery_company', ''),
            'delivery_street': order_data.get('delivery_street', ''),
        ...
        for item in order_items:
            converted_item = {
                ...
                'Rodzaj materiału': item.get('material', ''),
                'nawój/długość': item.get('roll_length', ''),
                'Średnica rdzenia': item.get('core', ''),
                'zam. ilość': item.get('ordered_quantity', ''),
                'Typ ilości': item.get('quantity_type', ''),
                'zam. rolki': item.get('zam_rolki', ''),
                'Cena': item.get('price', ''),
        ...
        export_order_to_pdf(order_dict, client_dict, converted_items, output_path)
        print(f"PDF wygenerowany pomyślnie: {output_path}")
        return True
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Użycie: python order_confirmation.py <plik_json> <ścieżka_wyjściowa_pdf>")
        sys.exit(1)
    
    json_file = sys.argv[1]
    output_file = sys.argv[2]
    
    success = generate_order_confirmation_from_json(json_file, output_file)
